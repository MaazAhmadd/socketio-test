...
makeRoute(
  ...
  async function (req, res) {
    ...
    publicRoomsWithFriendsJoined.length > 0 &&
      publicRoomsWithFriendsJoined.forEach(
        (r) => (r.entityId = (r as any)[EntityId]),
      );
    friendsRooms.length > 0 &&
      friendsRooms.forEach((r) => (r.entityId = (r as any)[EntityId]));
    invitedRooms.length > 0 &&
      invitedRooms.forEach((r) => (r.entityId = (r as any)[EntityId]));
